...
- **Annotation requirement on GCV params** (`pgparamannotation_test.go`): `GenericColumnValue` params whose `Type` differs only in `TypeAnnotation` pin that the annotation is **required** — plain `code:NUMERIC` / `code:JSON` are rejected with `Unimplemented "Unsupported GoogleSQL Type: NUMERIC/JSON"`, while `PG_NUMERIC` / `PG_JSONB` are accepted and echoed in result metadata. These findings back the godoc of `spanpg.EncodeOptions`.
- **spancodec end-to-end** (`spancodec_e2e_test.go`): struct rows → `spancodec.MutationColumnsAndValues` → `spanpg.InsertStatement` with params encoded via `spancodec.ValueOf(v, spanpg.EncodeOptions()...)` execute as DML and read back via `spancodec.ToStruct` **without** decode adaptation; the same row bound as plain Go values is rejected (the client encodes GoogleSQL forms). Also pins `spancodec.RowTypeFor` annotation inference and the `PG_NUMERIC` `NaN` decode hazard (`big.Rat` fails; `spanner.PGNumeric` works).
